...
export default function Archive() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      const fetchArchiveItems = async () => {
        try {
          const response = await fetch('https://ppexclusive-server.vercel.app/api/catalogue/archive');
  ...
  const handleDeleteArchiveItem = async (num: number) => {
    ...
    setIsLoading(false);
    if (response.ok) {
      setArchiveItems((prev) => prev.filter((item) => item.Num !== num));
    } else {
      console.error("Erreur lors de la suppression de l'élément de l'archive");
    }
  };
        ...
        {archiveItems.map((item) => (
          <li key={item.Num} className="items-center border p-4 rounded flex flex-col h-full">
            ...
            <div className="mt-auto flex flex-col items-center">
              ...
              <button
                onClick={() => handleDeleteArchiveItem(item.Num)}
